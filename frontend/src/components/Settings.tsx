...
export default function Settings() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 720 }}>
      <Title level={4} style={{ color: "#374151", marginBottom: 24 }}>
        ...
        <Row gutter={[16, 12]} style={{ marginTop: 12 }}>
          <Col span={8}>
            <Card size="small" bordered={false} style={{ background: backendOnline ? "#f0fdf4" : "#fef2f2", borderRadius: 8 }}>
              <Statistic
                title={<Text style={{ fontSize: 11, color: "#6b7280" }}>RAgent Router</Text>}
                value={backendOnline ? (lang === "zh" ? "运行中" : "Online") : (lang === "zh" ? "离线" : "Offline")}
                valueStyle={{ fontSize: 16, fontWeight: 600, color: backendOnline ? "#10b981" : "#ef4444" }}
                prefix={backendOnline ? <CheckCircleOutlined /> : <CloseCircleOutlined />}
              />
              <Text style={{ fontSize: 10, color: "#9ca3af" }}>:{backendPort}</Text>
            </Card>
          </Col>
          <Col span={8}>
            <Card size="small" bordered={false} style={{ background: ccDetails?.available ? "#f0fdf4" : "#fffbeb", borderRadius: 8 }}>
              <Statistic
                title={<Text style={{ fontSize: 11, color: "#6b7280" }}>CC Switch</Text>}
                value={ccDetails?.available ? (lang === "zh" ? "已连接" : "Connected") : (lang === "zh" ? "未检测到" : "Not Found")}
                valueStyle={{ fontSize: 16, fontWeight: 600, color: ccDetails?.available ? "#10b981" : "#f59e0b" }}
                prefix={ccDetails?.available ? <CheckCircleOutlined /> : <CloseCircleOutlined />}
              />
              {ccDetails?.db_size_mb !== undefined && (
                <Text style={{ fontSize: 10, color: "#9ca3af" }}>{ccDetails.db_size_mb} MB</Text>
              )}
            </Card>
          </Col>
          <Col span={8}>
            <Card size="small" bordered={false} style={{ background: trafficStatus?.available ? "#f0fdf4" : "#f9fafb", borderRadius: 8 }}>
              <Statistic
                title={<Text style={{ fontSize: 11, color: "#6b7280" }}>{lang === "zh" ? "流量数据" : "Traffic Data"}</Text>}
                value={trafficStatus?.total_records ?? 0}
                valueStyle={{ fontSize: 16, fontWeight: 600, color: "#6366f1" }}
                suffix={<Text style={{ fontSize: 10, color: "#9ca3af" }}>{lang === "zh" ? "条记录" : "records"}</Text>}
              />
              {trafficStatus?.last_request && (
                <Text style={{ fontSize: 10, color: "#9ca3af" }}>{trafficStatus.last_request}</Text>
              )}
            </Card>
          </Col>
        </Row>
          ...
          {ccDetails?.available ? (
            <>
              <Descriptions size="small" column={1} colon={false} style={{ marginBottom: 12 }}>
                <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "数据库路径" : "DB Path"}</Text>}>
                  <Text code style={{ fontSize: 11 }}>{ccDetails.path}</Text>
                </Descriptions.Item>
                {ccDetails.exe_exists && (
                  <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "程序路径" : "EXE Path"}</Text>}>
                    <Text code style={{ fontSize: 11 }}>{ccDetails.exe_path}</Text>
                  </Descriptions.Item>
                )}
                {ccStatus?.current_provider && (
                  <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "当前供应商" : "Current Provider"}</Text>}>
              ...
              {ccStatus?.proxy_configured ? (
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <Text style={{ fontSize: 11, color: "#6b7280" }}>{ccStatus.proxy_base_url}</Text>
      ...
      >
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0" }}>
            <div>
              <Text style={{ color: "#374151", fontSize: 13 }}>{t("settings.backendPort")}</Text>
              <br />
              <Text style={{ color: "#9ca3af", fontSize: 11 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0" }}>
            <div>
              <Text style={{ color: "#374151", fontSize: 13 }}>{t("settings.autoStart")}</Text>
              <br />
              <Text style={{ color: "#9ca3af", fontSize: 11 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0" }}>
            <div>
              <Text style={{ color: "#374151", fontSize: 13 }}>{t("settings.closeToTray")}</Text>
              <br />
              <Text style={{ color: "#9ca3af", fontSize: 11 }}>
        ...
        >
          ...
          <div style={{ marginTop: 12 }}>
            <Descriptions size="small" column={1} colon={false}>
              <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "应用数据" : "App Data"}</Text>}>
              ...
              <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "后端日志" : "Backend Logs"}</Text>}>
                <Text code style={{ fontSize: 11 }}>backend/logs/ragent.log</Text>
              </Descriptions.Item>
              <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "路由日志" : "Route Logs"}</Text>}>
                <Text code style={{ fontSize: 11 }}>backend/logs/routes.log</Text>
              </Descriptions.Item>
              <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "请求日志" : "Request Logs"}</Text>}>
                <Text code style={{ fontSize: 11 }}>backend/logs/requests.log</Text>
              </Descriptions.Item>
              <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "应用数据库" : "App Database"}</Text>}>
        ...
        >
          ...
          <div style={{ marginTop: 12 }}>
            <Descriptions size="small" column={2} colon={false}>
              ...
              <Descriptions.Item label={<Text style={{ color: "#9ca3af", fontSize: 11 }}>{lang === "zh" ? "开发模式" : "Dev Mode"}</Text>}>
